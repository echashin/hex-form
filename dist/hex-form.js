var hex = (function () {
    ...
    h.utils.exprToFunc = function (expr) {
        ...
        for (var v = 0, vl = variables.length; v < vl; v++) {
            ...
            if (variable !== '' && variable !== 'undefined') {
                var varAsArray = variable.split('.');
                var variableCan = variable.replace(/['"]/gi, '').replace(/\[/g, '.').replace(/]/g, '').replace(/\.+/g, '.').replace(/^\./, '').replace(/\.$/, '');

                if (functionVars.indexOf(varAsArray[0]) === -1) {
                    if (!/^this/ig.test(varAsArray[0])) {
                        functionVars.push(varAsArray[0]);
                    }
                }
        ...
        fnc1 = 'return (' + fnc1 + ');';
        return {'vars': vars, functionVars: functionVars, 'func': fnc1};
    };
    ...
    h.utils.objectExtend = function (originalObject, newObject) {
        ...
        for (var a in newObject) {
            ...
            var objectProperty = Object.getOwnPropertyDescriptor(originalObject, a);

            if (objectProperty === undefined || !$.isFunction(objectProperty.set)) {
